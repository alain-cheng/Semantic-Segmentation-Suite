...
# Set class weights
#class_weights = tf.constant([2.5, 0.5], dtype=tf.float32) # TEST
...
opt = tf.train.RMSPropOptimizer(learning_rate=learning_rate, decay=decay_rate).minimize(loss, var_list=[var for var in tf.trainable_variables()])
...
#model_checkpoint_name = args.model + "/" + "checkpoints/latest_mode_l" + args.model + "_" + args.dataset + ".ckpt"
model_checkpoint_name = args.model + "/" + "checkpoints/latest_model.ckpt"
...
for epoch in range(args.epoch_start_i, args.num_epochs):
    ...
    for i in range(num_iters):
        ...
        # Do the training
        _,current_loss=sess.run([opt,loss],feed_dict={net_input:input_image_batch,net_output:output_image_batch})
        ...
        if cnt % 20 == 0:
            string_print = "Epoch = %d Count = %d Current_Loss = %.4f Time = %.2f"%(epoch,cnt,current_loss,time.time()-st)
